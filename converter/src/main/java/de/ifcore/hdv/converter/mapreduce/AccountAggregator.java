...
public class AccountAggregator {
	...
	public static List<Account> reduceAreaKeyForCounty(Collection<Account> accounts) {
		...
		for (Account account : accounts) {
			...
			if (countyKey != null)
				result.add(new Account(countyKey, account.getAreaLabel(), account.getProductKey(), account
						.getProductName(), account.getAccountKey(), account.getAccountName(), account.getValue()));
		}
		return result;
	}
